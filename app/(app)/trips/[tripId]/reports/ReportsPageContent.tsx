...
export default function ReportsPageContent({ 
  ...
  const [includeFlightsInCountry, setIncludeFlightsInCountry] = useState(() => {
    ...
    const stored = localStorage.getItem(`reports:countryIncludeFlights:${tripId}`)
    return stored === 'true'
  })
  
  const [excludeFlightsFromDailyAvg, setExcludeFlightsFromDailyAvg] = useState(() => {
    if (typeof window === 'undefined') return false
    const stored = localStorage.getItem(`reports:dailyAvgExcludeFlights:${tripId}`)
    return stored === 'true'
  })
  
  const [excludeFlightsFromTotal, setExcludeFlightsFromTotal] = useState(() => {
    if (typeof window === 'undefined') return false
    const stored = localStorage.getItem(`reports:totalExcludeFlights:${tripId}`)
    return stored === 'true'
  })
