...
export class PrismaTripsRepository implements TripsRepository {
  ...
  async getTripForUser(tripId: string, userId: string): Promise<Trip | null> {
    // Temporarily disable caching until invite sharing is stable
  ...
  async createTrip(input: CreateTrip & { ownerId: string }): Promise<Trip> {
    const created = await prisma.trip.create({
      data: {
        ...
        ageRange: input.ageRange ? `AGE_${input.ageRange.toUpperCase()}` as any : null,
    ...
    return {
      ...
      ageRange: created.ageRange ? created.ageRange.replace('AGE_', '').toLowerCase() as any : undefined,
      targetBudget: created.targetBudget ?? undefined,
  ...
  async updateTrip(tripId: string, data: Partial<Trip>): Promise<Trip> {
    const updated = await prisma.trip.update({
      ...
      data: {
        ...
        ageRange: data.ageRange ? `AGE_${data.ageRange.toUpperCase()}` as any : undefined,
    ...
    return {
      ...
      ageRange: updated.ageRange ? updated.ageRange.replace('AGE_', '').toLowerCase() as any : undefined,
      targetBudget: updated.targetBudget ?? undefined,
